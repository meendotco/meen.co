<script lang="ts">
	...
	const candidates = [
		...
		{
			name: 'Michael Ross',
			position: 'Product Manager at Salesforce',
			image:
		...
		{
			name: 'Emily Johnson',
			position: 'Senior Product Manager at Dropbox',
			image:
		...
		},
		{
			...
			position: 'Product Manager at Adobe',
			image:
		...
		{
			name: 'James Wilson',
			position: 'Product Director at Slack',
			image:
...
<div id="about" class="relative z-10 mt-32 w-full max-w-screen-lg px-4">
	<Container delay={0.1} customClass="mb-10">
		<div class="mx-auto flex max-w-2xl flex-col items-center text-center">
			<SectionBadge title="Find the Perfect Match" />
			...
			<p class="mt-6 text-center text-base text-accent-foreground/80 md:text-lg">
				Find ideal candidates in seconds with our intelligent matching algorithm that understands
				your specific requirements and identifies the most qualified talent for your team
			</p>
		</div>
	</Container>
	<div
		...
		<div class="p-6 md:p-10">
			<div class="relative mx-auto max-w-2xl">
				...
				>
					<div class="flex flex-1 items-center">
						<svg
							class="ml-4 h-5 w-5 text-foreground/50 dark:text-white/50"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
				...
				<div class="mt-2 flex flex-wrap gap-2">
					...
					<span class="rounded-full bg-primary/20 px-2 py-1 text-xs font-medium text-primary"
						>5+ years</span
					>
					<span class="rounded-full bg-primary/10 px-2 py-1 text-xs text-primary">Remote</span>
				</div>
			</div>
			...
			<div class="mt-10 grid gap-6 sm:grid-cols-2 md:grid-cols-3">
				{#each candidates as candidate, index}
					...
					>
						<div
							class="from-neutral-80 pointer-events-none absolute inset-0 h-full w-full bg-gradient-to-t from-violet-950/25 to-transparent opacity-0 transition duration-200 group-hover/feature:opacity-100"
						/>
						<div
							class="relative z-10 flex transform-gpu items-center transition-all duration-300 group-hover/feature:-translate-y-1"
						>
							...
							<div>
								<h3 class="font-semibold text-foreground dark:text-white">{candidate.name}</h3>
								<p class="text-sm text-foreground/70 dark:text-white/70">
									{candidate.position}
								</p>
								<div class="mt-1 flex items-center">
									<span
										class={`rounded px-1.5 py-0.5 text-xs ${candidate.match >= 90 ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-100'}`}
									>
			...
			<div class="mt-6 flex justify-center">
				<button
					class="flex items-center gap-1 font-medium text-violet-600 transition-colors hover:text-violet-700"
				>
					View more results
					<svg
						class="h-4 w-4"
