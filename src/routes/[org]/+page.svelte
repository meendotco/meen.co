	...
	<div class="flex flex-col space-y-1.5">
		...
		{:else}
			<h1 class="text-2xl font-bold tracking-tight">
				{data.jobs[0]?.ownerOrganizationHandle || 'Organization'}
			</h1>
	...
	<div class="grid grid-cols-1 gap-5 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
		...
		{:else}
			{#each jobs as job}
				...
				>
					<h3 class="mb-2 line-clamp-1 text-lg font-medium">{job.title}</h3>
					...
					<div class="mb-4 flex flex-wrap gap-2 text-xs">
						...
						{#if job.type}
							<span class="rounded-md bg-[#27272A] px-2 py-1">
								{job.type}
							</span>
						{/if}
					</div>
					...
					{#if job.priority === 'High'}
						<div class="absolute right-2 top-2">
							<span
								class="rounded-full bg-[#6D2EE0]/20 px-2 py-0.5 text-xs font-medium text-[#7E3AF2]"
							>
								{job.priority}
							</span>
						</div>
